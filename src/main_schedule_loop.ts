...
export async function refreshHourlyJobForUserAssign(sentryCheckInId: string ): Promise<boolean> {
  ...
  try {
    ...
    log.err('***REMOVED***runHouerlyJobForUserAssign', (err as Error).stack);
...
export async function mainHourlyJob () {
  ...
  try {
    ...
    if (isSuccessForAnnBtnClick && userIdMapingStatus) {
      ...
      const errMsg = 'Any one of the crawler did not successfully run, so ***REMOVED***REFRESH_LEAD_ACTIVITY did not run';
      log.err(errMsg);
      throw new Error(errMsg);
    }
  } catch (err) {
    log.err('***REMOVED***mainHourlyJob', (err as Error).stack);
    captureException(err as Error);
  }
}
...
export async function runHouerlyJob(sentryCheckInId: string) {
  try {
    ...
    log.err('***REMOVED***runHouerlyJob', (err as Error).stack);
    captureException(err as Error);
  }
}
...
async function runRollup() {
  ...
  try {
    ...
    log.err('***REMOVED***runRollup', (err as Error).stack);
    captureException(err as Error);
  }
}
